...
async def upsert_profile(telegram_id: int, auth_code: str, extra: dict | None = None) -> bool:
    ...
    try:
        async with httpx.AsyncClient(timeout=15) as c:
            ...
            if exists:
                # Оновлюємо тільки auth_code і поля з extra; не чіпаємо document_number/tax_number якщо вже є
            ...
            # Якщо UPDATE і document_number/tax_number порожні — заповнити
            if exists:
                patch2 = {}
...
async def update_profile(telegram_id: int, data: dict) -> bool:
    try:
        async with httpx.AsyncClient(timeout=15) as c:
            ...
            if not exists:
                # Створити рядок якщо його немає
...
async def upload_file(file_bytes: bytes, bucket: str, path: str, content_type: str) -> str | None:
    try:
        ...
        async with httpx.AsyncClient(timeout=30) as c:
            r = await c.post(url, headers=headers, content=file_bytes)
